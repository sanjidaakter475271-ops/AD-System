...
import { Monitor, ArrowLeft, Save, Sparkles, CheckCircle2, AlertCircle, Trash2, Building, Plus } from 'lucide-react';
...
export default function EditEquipmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    async function loadItem() {
      try {
        ...
        if (!res.ok) throw new Error('Equipment record not found');
        const item = await res.json();
        setFormData({
          sn: item.sn || 0,
          baseUnit: item.baseUnit || 'Air HQ',
          directorate: item.directorate || DIRECTORATES[0],
          equipmentType: item.equipmentType || EQUIPMENT_TYPES[0],
          brandModel: item.brandModel || '',
          serialNo: item.serialNo || '',
          processor: item.processor || PROCESSORS[0],
          generation: item.generation || 8,
          ramGb: item.ramGb || 8,
          ssdGb: item.ssdGb || 0,
          hddGb: item.hddGb || 0,
          status: item.status || STATUS_OPTIONS[0],
          location: item.location || '',
          issueStatus: item.issueStatus || ISSUE_STATUS_OPTIONS[0],
          isNewPc: Boolean(item.isNewPc),
          adStatus: item.adStatus || 'Pending',
          adRemark: item.adRemark || '',
          win10Remark: item.win10Remark || '',
        });
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Equipment record updated successfully!');
      setTimeout(() => {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              ...
              Edit Equipment #{formData.sn}
            </h1>
            <p className="text-sm text-slate-400">Update equipment record in Active Directory & Inventory database</p>
          </div>
        </div>
        ...
        <button
          ...
          disabled={deleting}
          className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-rose-500/20 text-rose-300 border border-rose-500/40 hover:bg-rose-500/30 text-xs font-semibold transition-all"
        >
      ...
      <form onSubmit={handleSubmit} className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
        ...
        <div>
          <h2 className="text-sm font-bold text-sky-400 uppercase tracking-wider mb-4 border-b border-slate-800 pb-2 flex items-center justify-between">
            <span>1. Location, Base/Unit & Basic Info</span>
            <div className="flex items-center gap-3">
              {isAdmin && (
                <button
                  type="button"
                  onClick={() => setShowAddBaseModal(true)}
                  className="text-xs font-semibold text-sky-400 hover:text-sky-300 flex items-center gap-1 capitalize font-normal"
                >
              ...
              <button
                ...
                className="text-xs font-semibold text-indigo-400 hover:text-indigo-300 flex items-center gap-1 capitalize font-normal"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1">SN (Serial Index)</label>
              <input
                type="number"
                value={formData.sn}
                onChange={(e) => setFormData({ ...formData, sn: parseInt(e.target.value) || 0 })}
                className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-sky-500"
                required
              />
            </div>
